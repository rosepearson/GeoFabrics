...
class BaseProcessor(abc.ABC):
    ...
    def get_instruction_general(self, key: str):
        ...
        if (
            "general" in self.instructions
            and key in self.instructions["general"]
        ):
...
class MeasuredRiverGenerator(BaseProcessor):
    ...
    def estimate_river_mouth_fan(self, defaults: dict):
        ...
        fan_polygon.to_file(
            self.get_instruction_path("fan_polygon", defaults=defaults)
        )
    ...
    def run(self):
        ...
        if self.get_measured_instruction("estimate_fan"):
            ...
            if not (
                fan_polygon_file.exists() and fan_elevations_file.exists()
            ):
